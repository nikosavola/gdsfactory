...
INTERDIGITAL_DEFAULTS = {k: v.default for k, v in inspect.signature(interdigital_capacitor).parameters.items()}
...
def interdigital_capacitor_enclosed(
    ...
    """
    c = Component()
    cap = interdigital_capacitor(fingers, finger_length, finger_gap, thickness, metal_layer).ref_center()
    c.add(cap)
    ...
    for port in cap.get_ports_list():
        ...
        x = gf.CrossSection(width=cpw_a, offset=0, layer=metal_layer, port_names=("in", "out"), sections=[s1, s2])
        ...
        term = c << gf.components.bbox([[0, 0], [cpw_b, cpw_a + 2 * cpw_b]], layer=gap_layer)
        if direction < 0:
            term.movex(-cpw_b)  # TODO approx?
        term.move(destination=route.ports[-1].move_copy(-1 * np.array([0, cpw_a / 2 + cpw_b])))
    ...
    ground = gf.geometry.boolean(A=ground, B=[c, gap], operation="A-B", layer=metal_layer)
